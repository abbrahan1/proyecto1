...
<button class="btn btn-sm" style="margin: 10px; padding: 10px; text-decoration: none; background-color: #F9E79F; color: ;"><a href="cargahorario.php" style="color: black; text-decoration: none;">cargar nuevo horario</a></button>

<div class="row mx-auto" style="padding-bottom: 20px;">
    <div class="col-md-10 col-sm-12 mx-auto text-center">
        <h2 class="text-center mb-4" style="font-family: 'Comic Sans MS', sans-serif;">Horario</h2>
        ...
        <?php
        include('conex.php');
        include('select_hor.php');
        while ($key = mysqli_fetch_array($res)) {
            echo '
            <tr>
                <td>' . htmlspecialchars($key['sala']) . '</td>
                <td>' . htmlspecialchars($key['turno']) . '</td>
                <td>' . htmlspecialchars($key['maestra']) . '</td>
            </tr>
            ';
        }
        ?>
